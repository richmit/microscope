...
file:dialog-settings.gif
 ...
 - file:toolsetbutton-video.gif      :: Live video feed from camera.  Note the scale setting for the live video feed is diffrent from the capture preview scale.

If "Change Settings Before Capture" is set, then a simplified settings dialog box will appear before the video feed window.

#+ATTR_HTML: :align center :width 400px
file:dialog-settings-v.gif
