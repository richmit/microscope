...
is such a dynamic project, things change.  For example, the day I installed it I had to work around a bug with java =accessibility.properties=.
...
sudo reboot
#### Install packages
sudo apt install imagemagick nomacs gimp exiv2
...
sudo cp ImageJ.sh /usr/bin/ImageJ
chmod a+rx /usr/bin/ImageJ
...
#### Create directory that will hold camera images
mkdir -p ~/Pictures/pi-cam/
#### Clone the git repo
#### Note: We link to files in the repo, so if you do a git pull later everything gets updated in place.
cd ~
