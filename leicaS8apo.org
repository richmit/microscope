...
|          <r> | <l>                         |      |   <r> | <l>             | <c>  |
|            / | <>                          |      |       |                 |      |
|     10445928 | Leica Video Objective 0.32x | 0.32 |   6.7 | Estimated $O_d$ | co32 |
|     10450528 | Leica Video Objective 0.5x  | 0.50 |  10.5 | Measured $O_d$  | co50 |
|     10447367 | Leica Video Objective 0.63x | 0.63 |  13.2 | Estimated $O_d$ | co63 |
|     10446307 | Leica Video Objective 0.8x  | 0.80 |  16.8 | Estimated $O_d$ | co80 |
#+TBLFM: @4$4..@>$4=21*$3;%.1f

Note the image circles are all the same size on the image sensor for a particular video objective, but FOV will change with the Auxiliary & video objective.
Some examples:

#+CAPTION: Image Circles with $O_d=0.5x$
...
convert 10450528-10446335-z1.jpg -rotate 180 -pointsize 100 -draw "gravity northwest fill white text 10,10 'Z=1  A=0.63x  C=0.5x'" -draw "gravity northwest fill red text 4300,1944 '32 mm'"  -draw "gravity southeast fill blue text 1700,1239 'RPI HQ'" -draw "gravity southwest fill green text 4350,10 'Micro Four Thirds'" -fill none -stroke red -strokewidth 10 -draw 'circle 2592,1944 4093,1944' -stroke blue -draw 'rectangle 1655,1239 3529,2649' -stroke green -draw 'rectangle 1,1 5184,3888' 10450528-10446335-z1.png; 
echo anno 10450528-10446335-z8.jpg
convert 10450528-10446335-z8.jpg -rotate 180 -pointsize 100 -draw "gravity northwest fill white text 10,10 'Z=8  A=0.63x  C=0.5x'" -draw "gravity northwest fill red text 4300,1944 '4 mm'"   -draw "gravity southeast fill blue text 1700,1239 'RPI HQ'" -draw "gravity southwest fill green text 4350,10 'Micro Four Thirds'" -fill none -stroke red -strokewidth 10 -draw 'circle 2592,1944 4093,1944' -stroke blue -draw 'rectangle 1655,1239 3529,2649' -stroke green -draw 'rectangle 1,1 5184,3888' 10450528-10446335-z8.png; 
echo anno 10450528-NONE-z1.jpg  
convert 10450528-NONE-z1.jpg     -rotate 180 -pointsize 100 -draw "gravity northwest fill white text 10,10 'Z=1  A=1.00x  C=0.5x'" -draw "gravity northwest fill red text 4300,1944 '20 mm'"  -draw "gravity southeast fill blue text 1700,1239 'RPI HQ'" -draw "gravity southwest fill green text 4350,10 'Micro Four Thirds'" -fill none -stroke red -strokewidth 10 -draw 'circle 2592,1944 4093,1944' -stroke blue -draw 'rectangle 1655,1239 3529,2649' -stroke green -draw 'rectangle 1,1 5184,3888' 10450528-NONE-z1.png; 
echo anno 10450528-NONE-z8.jpg
convert 10450528-NONE-z8.jpg     -rotate 180 -pointsize 100 -draw "gravity northwest fill white text 10,10 'Z=8  A=1.00x  C=0.5x'" -draw "gravity northwest fill red text 4300,1944 '2.5 mm'" -draw "gravity southeast fill blue text 1700,1239 'RPI HQ'" -draw "gravity southwest fill green text 4350,10 'Micro Four Thirds'" -fill none -stroke red -strokewidth 10 -draw 'circle 2592,1944 4093,1944' -stroke blue -draw 'rectangle 1655,1239 3529,2649' -stroke green -draw 'rectangle 1,1 5184,3888' 10450528-NONE-z8.png; 
...
I use two cameras with my S8APO:
  - [[https://www.getolympus.com/us/en/digitalcameras/omd/e-m1-mark-ii.html][Olympus OM-D E-M1 Mark II]] with a 0.5x or 0.32x video objective
  - [[#rpi-camera][A DIY solution]] with a Raspberry Pi & Raspberry Pi HQ Camera attached to a 0.32x video objective.
...
** System Setup With Custom Home Directory                         :noexport:
...
#### Create my account
addgroup richmit
adduser -G 1001 richmit ## use GID from previous command
...
wget 'https://github.com/imagej/imagej2/raw/master/bin/ImageJ.sh'
#### Unpack Fiji
unzip fiji-nojre.zip
#### Put in place ImageJ.sh script
...
sudo sh -c 'grep -v ^assistive_technologies /etc/java-8-openjdk/accessibility.properties.org > /etc/java-8-openjdk/accessibility.properties'
...
** Camera Data Sync With Custom Home Directory                     :noexport:
...
cp piSnap.sh ~/bin
cd ~/bin
ln -s piSnap.sh piSnap
cd ~/microscope
### EDIT the install script for your install path!!!  Then run it
./RPI_tools_install.sh
#+end_src

** Camera Data Sync

#+begin_src sh
date; echo back-dat-sync;        rsync -rlt --log-format=%f --delete --delete-excluded --modify-window=2                  --rsh='ssh' pi-cam.home.mitchr.me:synced/pi-data/                            /c/Users/richmit/Documents/world/pi-cam/; date
#+end_src
...
#+ATTR_HTML: :align center
file:toolsetmenu_800.gif

Once activated, it will add three buttons to the toolbar:

#+ATTR_HTML: :align center
file:toolsetbuttons_800.gif

Button Functions:
 - file:toolsetbutton-capture.gif :: Capture, save, and open an image from the camera.  Images are stored in the same location, and with the same file name conventions, used by =piSnap.sh=.
 - file:toolsetbutton-load.gif    :: Load last capture(s) from camera using the ImageJ/Fiji macro above or the =piSnap.sh= script.  If the last capture session with =piSnap.sh= captured multiple images, then *all* of them will be loaded.
 - file:toolsetbutton-scale.gif   :: Set image scale based upon microscope settings.  This script contains hardwired settings for my specific microscope and lenses, and may require modification for your microscope.

#+INCLUDE:RPI_tools.ijm src c

*** Macro Installer

I use Fiji on Windows & [[#rpi-camera][DIY RPI Camera]].  On Windows I install it under a folder named "=PF/Fiji.app=" -- I install various packages in "=PF="
